	...
	world_distance new_ceiling_height)
{
	...
	#pragma unused (new_ceiling_height)
...
#ifdef OBSOLETE
/* yaw must be in [0,2π), pitch must be in [-π/2,π/2], velocity must be in [-1,1]; returns new
	...
	long action_flags)
{
	...
	short sine, cosine;
	fixed delta_z;
	fixed delta; /* used as a scratch ‘change’ variable */
